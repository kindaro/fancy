...
class Bristle arrows sources targets
  | arrows → sources, arrows → targets, sources targets → arrows
  where bristle ∷ arrows → sources → targets
instance Bristle ( ) ( ) ( ) where bristle ( ) ( ) = ( )
instance Bristle arrows sources targets ⇒ Bristle ((source → target) × arrows) (source × sources) (target × targets)
  where bristle (arrow :×  arrows) (source :× sources ) = arrow source :× bristle arrows sources
...
class transposed ~ Transposed outer tuple ⇒ TransposeTuple outer tuple transposed
  | outer tuple → transposed where transposeTuple ∷ outer tuple → transposed
instance TransposeTuple functor ( ) ( ) where transposeTuple _= ( )
instance (TransposeTuple functor transposable transposed, Functor functor)
  ⇒  TransposeTuple functor (value, transposable) (functor value, transposed)
  where transposeTuple functor = (fmap fst functor, (transposeTuple ∘ fmap snd) functor)
